		...
		while wait() do
			pcall(function()
				if game.PlaceId ~= 4442272183 then
					...
					if havemelee() ~= 0 and havemelee() ~= 1 then
						if getdarkfrag() < 2 then
							if not haveDarkbeard() then
								local allData = getinfoallboss2()["data"]
								if #allData > 1 then
									ContentSet(
										"Get Dark Fragment ( Crawl Server : " .. allData[#allData]["Rare Boss"] .. " )",
										data())

									if (allData[#allData]["Rare Boss"] == "Darkbeard") then
										joinserver(allData[#allData]["JobId"], false)
									end
								end
							else
								ContentSet("Have Darkbeard", data())
								if haveDarkbeard() and haveDarkbeard():FindFirstChild("HumanoidRootPart") and haveDarkbeard():FindFirstChild("Humanoid") then
									ContentSet("Kill Darkbeard", data())
									--Tweento(haveDarkbeard().HumanoidRootPart.CFrame * CFrame.new(0, 20, 0))
									--task.wait(1)
									repeat
										wait()
										pcall(function()
											spawn(function()
												if haveDarkbeard() and haveDarkbeard():FindFirstChild("HumanoidRootPart") and haveDarkbeard():FindFirstChild("Humanoid") then
													Tweento(haveDarkbeard().HumanoidRootPart.CFrame *
														CFrame.new(0, 20, 0))
												end
											end)
											spawn(function()
									...
									until not haveDarkbeard() or not haveDarkbeard().Parent or not haveDarkbeard():FindFirstChild("Humanoid") or not haveDarkbeard():FindFirstChild("HumanoidRootPart") or haveDarkbeard().Humanoid.Health <= 0 or not islive()
									getgenv().Enabled = false
								end
							end
						else
							if getVampireFang() < 20 then
								farmnoboss = true
							else
								farmnoboss = false
								if game:GetService("Players").LocalPlayer.Data.Fragments.Value >= getgenv().Config["Frag Limit"] then
						...
						if havemelee() == 0 then
							...
							ContentSet("Have Sanguine Art", data())
							writefileyummy()
						end
					end
				end
